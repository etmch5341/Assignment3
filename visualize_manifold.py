...
def generate_tsne_visualization(model_path, data_dir, output_file, sample_size=1000):
    ...
    # We need the full dataset object to access file paths for labeling
    ...
    for idx in tqdm(indices, desc="Encoding samples"):
        ...
        # Get the source file path to create a label (e.g., subject ID)
        file_path, _ = dataset.windows[idx]
        # The subject ID is typically the second-to-last part of the path
    ...
    label_codes, unique_labels = pd.factorize(all_labels)

    scatter = plt.scatter(tsne_results[:, 0], tsne_results[:, 1], c=label_codes, cmap='jet', alpha=0.7)
...
if __name__ == "__main__":
    generate_tsne_visualization(
        ...
        data_dir="./cmu-mocap",
        output_file="./output/ae/plots/manifold_visualization.png"
    )
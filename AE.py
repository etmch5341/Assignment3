...
class MotionManifoldSynthesizer:
    ...
    def _compute_gram_matrix(self, H: torch.Tensor) -> torch.Tensor:
        """
        ...
        gram_matrix = torch.bmm(H, H.transpose(1, 2))
        # Normalize by the number of time steps as described in the paper
    ...
    def transfer_style(
        ...
        """
        Transfers the style of one motion to the content of another using the method
        ...
        # Helper to format a motion sample into a model-ready input tensor
        def prepare_input(motion):
            positions = motion["positions_normalized_flat"].unsqueeze(0).to(self.device)
            trans_vel = motion["trans_vel_xz"].unsqueeze(0).to(self.device)
            rot_vel = motion["rot_vel_y"].unsqueeze(0).to(self.device).unsqueeze(-1)
        ...
        # 1. Pre-compute target content and style representations in the latent space
        with torch.no_grad():
            content_input = prepare_input(content_motion)
            style_input = prepare_input(style_motion)
            
            H_content = self.model.encode(content_input)
            H_style = self.model.encode(style_input)
            
            # The target style is the Gram matrix of the style motion's latent representation
            G_style_target = self._compute_gram_matrix(H_style)

        # 2. Initialize the latent variable to be optimized from white noise
        H_optim = torch.randn_like(H_content, device=self.device, requires_grad=True)
        
        # 3. Setup optimizer
        optimizer = optim.Adam([H_optim], lr=learning_rate)

        # 4. Optimization loop to find the H that minimizes content and style losses
        print("Optimizing for style transfer...")
        for i in tqdm(range(num_iterations), desc="Style Transfer Opt."):
            optimizer.zero_grad()
            
            # Calculate the Gram matrix of the current optimized latent variable
            G_current = self._compute_gram_matrix(H_optim)
            
            # Content Loss: Measures how much the latent codes differ.
            # We want the optimized latent code H_optim to be close to the content's latent code.
            L_content = F.mse_loss(H_optim, H_content)
            
            # Style Loss: Measures how much the feature correlations (style) differ.
            # We want the Gram matrix of H_optim to be close to the style's Gram matrix.
            L_style = F.mse_loss(G_current, G_style_target)
        ...
        # 5. Decode the final optimized latent representation back to motion space
        with torch.no_grad():
            result_motion_norm_flat = self.model.decode(H_optim)

        # 6. Isolate, reshape, and un-normalize the position data to get the final motion
        pos_flat = result_motion_norm_flat[:, :, :content_motion["positions_normalized_flat"].shape[1]]
        b, t, _ = pos_flat.shape
        j, d_pos = self.mean_pose.shape
        
        result_normalized = pos_flat.reshape(b, t, j, d_pos)
        result_motion_local = (result_normalized * self.std + self.mean_pose).squeeze(0)
        
        return result_motion_local.cpu()
